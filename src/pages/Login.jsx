...
const PageHeader = ({ eyebrow, title, sub }) => (
  <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:.7 }}
    ...
    <p className="au-l" style={{ color:"#c69e8f", fontSize:10, letterSpacing:".45em", textTransform:"uppercase", fontWeight:700, marginBottom:14 }}>{eyebrow}</p>
    <h1 className="au-d" style={{ color:"#6d4e19", fontSize:"clamp(24px,4vw,44px)", fontWeight:700, marginBottom:14 }}>{title}</h1>
    <div className="au-div" style={{ width:90, margin:"0 auto 18px" }} />
    <p className="au-s" style={{ color:"#3d3228", fontSize:"clamp(14px,1.8vw,18px)", fontStyle:"italic", maxWidth:440, margin:"0 auto", lineHeight:1.85 }}>{sub}</p>
  </motion.div>
);

const Card = ({ children, delay=0.2 }) => (
  <motion.div initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ duration:.7, delay }}
    style={{ background:"white", border:"1px solid rgba(188,193,194,0.35)", padding:"40px 36px" }}>
    {children}
  </motion.div>
);

const Field = ({ label, inputRef, type="text", placeholder }) => (
  <div style={{ marginBottom:18 }}>
    <label className="lbl">{label}</label>
    <input ref={inputRef} type={type} className="au-input" placeholder={placeholder} />
  </div>
);
...
const Login = () => {
  ...
  const loginUser = () => {
    ...
    axios.post("http://localhost/Jewellerydb/signin.php", formData)
      .then((res) => {
        if (res.data.status === "true") {
          ...
          axios.post("http://localhost/Jewellerydb/sendmail.php", mailData)
            .then(() => console.log("Mail Sent")).catch(() => console.log("Mail server error"));
  ...
  return (
    <>
      ...
      <div style={{ background:"#f5f6f5", minHeight:"100vh" }}>
        <PageHeader eyebrow="✦ Members Only ✦" title="Access Your Vault" sub="Welcome back — your curated collection awaits." />
        <div style={{ maxWidth:460, margin:"0 auto", padding:"52px 24px 96px" }}>
          <AnimatePresence>
            {showSuccess && (
              <motion.div className="au-ok" initial={{ opacity:0, y:-10 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}>
                <CheckCircle size={18} color="#6d4e19" />
                <span className="au-d" style={{ fontSize:11, letterSpacing:".15em", color:"#6d4e19", textTransform:"uppercase" }}>Login Successful</span>
              </motion.div>
            )}
          </AnimatePresence>
          <Card>
            <p className="au-l" style={{ color:"#c69e8f", fontSize:10, letterSpacing:".4em", textTransform:"uppercase", fontWeight:700, marginBottom:14 }}>✦ Sign In ✦</p>
            <h2 className="au-d" style={{ color:"#6d4e19", fontSize:20, fontWeight:700, marginBottom:8 }}>Welcome Back</h2>
            <div className="au-div" style={{ width:50, marginBottom:28 }} />
            <Field label="Email" inputRef={emailRef} type="email" placeholder="name@example.com" />
            <div style={{ marginBottom:28 }}>
              <label className="lbl">Password</label>
              <input ref={passwordRef} type="password" className="au-input" placeholder="Your password" />
            </div>
            <button className="au-btn" onClick={loginUser} disabled={loading}>
              {loading ? "Accessing..." : "✦ Access Vault"}
            </button>
            <p className="au-l" style={{ textAlign:"center", marginTop:22, fontSize:12, color:"#74878b", fontWeight:700 }}>
              New here? <Link to="/register" className="au-lnk">Join Our Exclusive Circle</Link>
            </p>
          </Card>
        </div>
      </div>
